...
- **Gotchas** (detailed in the docs): consult the `nuxt-ui-remote` MCP before using a new Nuxt UI component; relative TS imports have **no `.js`** extension; markdown via `<AppMarkdown>` (never `@nuxtjs/mdc`).
