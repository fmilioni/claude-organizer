...
- **(b) Every review finding that was NOT fixed** — each with why (deferred / too-big), and the too-big ones pointing at the **inbox items** you created. **"It was only `low`" is not a valid why** — a `low` in-scope finding should have been fixed in the loop, so if one lands here, it's because the user explicitly dismissed it or it was genuinely too big, never because you judged it not worth fixing.
