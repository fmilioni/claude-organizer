...
- **Then dispose of the findings** — fix in scope (a new commit on the story, attached like any other), and for anything you don't fix offer **per finding**: **send it to the inbox** (run the `claude-organizer` dedup-check first — search the board + pending inbox; reference an existing card/item instead of duplicating) or **create a follow-up card**.
